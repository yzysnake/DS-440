...
def model_info_label():
    try:
        with open('model_info.json', 'r') as openfile:
            model_info = json.load(openfile)
            model_info_text = "Running on {} algorithm with {} data size and \n{}"
            model_info_text = model_info_text.format(str(model_info.get('algorithm')), str(model_info.get('data size')),
                                                     str((model_info.get('indicator'))))
            ####### model information label ######
            model_label = tk.Label(topLeftFrame, text=model_info_text, padx=10)
            model_label.config(font=("Helvetica 16 bold italic", 16))
